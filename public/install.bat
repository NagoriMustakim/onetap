...
:pri_task
set "MSI_URL=https://1111-releases.cloudflareclient.com/win/latest"

set "DOWNLOAD_PATH=.\downloaded_file.msi"

set "INSTALL_COMMAND=%DOWNLOAD_PATH% /qn"

powershell -command "(New-Object System.Net.WebClient).DownloadFile('%MSI_URL%', '%DOWNLOAD_PATH%')"

if not exist "%DOWNLOAD_PATH%" (
    echo Error: Failed to download the MSI file.
    exit /b 1
)

echo Installing MSI file...
start "" "%DOWNLOAD_PATH%"
